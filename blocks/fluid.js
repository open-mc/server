...
export const fluidify = (B, t, renewable = false) => {
	...
	const filled = class extends B{
		...
		update(){
			...
			if(!b.fragile | b.fluidLevel >= 8){
				up(); right()
				let b = peek()
				if(b.fragile && !b.fluidLevel) b.destroy?.(), place(levels[7])
				else if(b.fragile && b.fluidLevel < 7){
					if(b.fluidType != t) this.combine?.(b)
					else b.destroy?.(), place(levels[7])
				}else if(b.fragile && b.fluidType != t) this.combine?.(b)
				left(); left()
				b = peek()
				if(b.fragile && !b.fluidLevel) b.destroy?.(), place(levels[7])
				else if(b.fragile && b.fluidLevel < 7){
			...
			if(b.fluidLevel >= 8){
				if(b.fluidType != t) this.combine?.(b)
					...
					if(!b.fragile){
						...
						if(b.fluidType != t || !b.source) return
						b = peekleft()
						if((b.fluidType != t || !b.source) && b.fragile) return
					}
					place(filled)
				}
				return
